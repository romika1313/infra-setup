...
// Helper: builds a Key Vault app-setting reference for the latest secret version
var kv = (secretName) => '@Microsoft.KeyVault(SecretUri=${keyVaultUri}secrets/${secretName}/)'
  ...
  properties: {
    ...
    siteConfig: {
      ...
      appSettings: [
        ...
        // These are resolved at runtime; the app refuses to start if required ones are missing
        { name: 'DATABASE_URL',                      value: kv('database-url') }
        { name: 'NEXTAUTH_SECRET',                   value: kv('nextauth-secret') }
        { name: 'AZURE_AD_CLIENT_SECRET',            value: kv('azure-ad-client-secret') }
        { name: 'AZURE_STORAGE_CONNECTION_STRING',   value: kv('storage-connection-string') }
        { name: 'DOCUMENT_INTELLIGENCE_KEY',         value: kv('document-intelligence-key') }
        { name: 'AZURE_OPENAI_API_KEY',              value: kv('azure-openai-api-key') }
        { name: 'AZURE_MAPS_KEY',                    value: kv('azure-maps-key') }
        { name: 'INGEST_SHARED_SECRET',              value: kv('ingest-shared-secret') }
        { name: 'DIGEST_SHARED_SECRET',              value: kv('digest-shared-secret') }
      ]
    }
  }
}
