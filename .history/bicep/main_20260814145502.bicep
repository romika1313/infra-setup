...
// Runs after rbac so the deployment service principal can write secrets
var storageConnectionString = 'DefaultEndpointsProtocol=https;AccountName=${storageAccountName};AccountKey=${listKeys(storage.outputs.id, '2023-05-01').keys[0].value};EndpointSuffix=core.windows.net'
var databaseUrl = 'sqlserver://${sql.outputs.serverFqdn}:1433;database=${sqlDatabaseName};user=${sqlAdminLogin};password=${sqlAdminPassword};encrypt=true;trustServerCertificate=false;loginTimeout=30'
var docIntKey = listKeys(docIntelligence.outputs.id, '2023-05-01').key1
var openAiKey = listKeys(openAi.outputs.id, '2023-05-01').key1
var mapsKey   = listKeys(maps.outputs.id, '2023-06-01').primaryKey
